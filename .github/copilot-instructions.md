...
- **运行测试**:
  - **开发时**: `npm test` (监听模式，需要手动按 q 退出)
  - **自动化/CI**: `npm run test:run` (单次运行，自动退出，⚠️ 推荐用于无人工干预场景)
  - **覆盖率检查**: `npm run test:coverage` (自动退出)
- **新文件规则**: 
...
- **Bug 清单**: 所有已知但未修复的问题记录在 `docs/bugs` 文件中
- **状态标记**: 使用 `[ ]` 表示未解决，`[x]` 表示已解决
- **修复优先级**:
  - **P0 (紧急)**: 影响核心功能、数据安全、用户体验的严重问题，立即修复
  - **P1 (重要)**: 影响次要功能或用户体验的问题，优先处理
  - **P2 (一般)**: 优化类问题，可以在相关代码开发时一并解决
  - **P3 (低)**: 不影响使用的小问题，有时间再处理
- **处理策略**:
  - 在每次会话开始时，检查 `docs/bugs` 文件中的未解决问题
  - 当开发或修改相关代码时，主动检查是否有关联的 bug 可以一并解决
  - 修复 bug 后，及时在 `docs/bugs` 中标记为已解决 `[x]`
  - 复杂的 bug 修复需要创建专门的文档记录修复过程
- **提醒时机**:
  - 用户询问某个功能时，检查是否有相关的已知 bug
  - 开发涉及某个模块时，提醒用户该模块是否有待修复的 bug
  - 适当时候建议集中处理一批相关的 bug
