...
### 测试覆盖
- **单元测试**: 32个测试用例全部通过 ✅
- **代码覆盖率**: 93.33% (lines), 81.63% (branches), 100% (functions)
- **集成测试**: 通过全局1930个测试,无回归问题 ✅
- **测试场景**:
  - ≤3篇文章跳过初筛 ✓
  - 4篇文章开始初筛 ✓
  - 10篇文章成功初筛 ✓
...
### 配置参数 (优化后)
- **最大输入tokens**: 80K (适配主流128K上下文窗口模型)
- **最大输入字节**: 320KB (约4字节/字符估算)
- **单次最多文章数**: 100篇
- **Description截断**: 800字符（提供更多上下文）
- **最小初筛文章数**: 4篇（≤3篇跳过初筛，直接全量分析）
- **筛选率阈值**: 10%-90% (低于或高于此范围视为异常)
