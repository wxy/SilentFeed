...
**设计理念**：TF-IDF 预筛选 + AI 精准评分的混合推荐系统
...
├─────────────────────────────────────────────────────────┤
│                                                           │
│  第一层: TF-IDF 预筛选（本地，快速）                       │
│  ├─ 计算用户兴趣画像与文章的相似度                          │
│  ├─ 过滤明显不相关的文章                                   │
│  └─ 降低 AI 调用成本                                      │
│  优点: 完全本地、零成本、隐私友好                           │
│                                                           │
│  第二层: AI 精准评分（用户 API，必需）                     │
│  ├─ OpenAI (gpt-4o, gpt-4o-mini)                        │
│  ├─ Anthropic (claude-3.5-sonnet, claude-3.5-haiku)    │
│  └─ DeepSeek (deepseek-chat, deepseek-reasoner)        │
│  优点: 准确度高、隐私可控（用户自己的 API Key）            │
│  准确率: ~85-90%                                          │
│                                                           │
│  本地 AI 选项:                                            │
│  • Ollama（推荐，立即可用）                               │
│    - 支持 Llama 3.2、Qwen 2.5、Gemma 2 等开源模型        │
│    - 完全本地运行，免费无限制                             │
│    - 参考: docs/OLLAMA_SETUP_GUIDE.md                    │
│                                                           │
│  未来计划:                                                │
│  • Chrome AI（Gemini Nano）- 待 API 稳定后支持            │
│                                                           │
└─────────────────────────────────────────────────────────┘
```
...
AI 评估:
- Input: ~3000 tokens（用户画像 + 文章摘要）
- Output: ~500 tokens（评分结果和理由）
- Total: ~3500 tokens/天
...
### 5.3 Chrome AI 支持（未来计划）

> **状态**: 🚧 暂未实现  
> **原因**: Chrome AI (Gemini Nano) 目前仍在实验阶段，API 不稳定，平台支持受限（macOS 不可用）  
> **计划**: 待 Chrome 138+ 稳定版普及、API 正式发布后再添加支持

Chrome 内置的 Gemini Nano 模型将成为另一个隐私友好的本地 AI 选项。未来实现时的设计草案：

```typescript
// 未来实现的设计草案（仅供参考）
// src/core/ai/ChromeAIAdapter.ts (暂未创建)
...
**当前推荐**: 使用 [Ollama](./OLLAMA_SETUP_GUIDE.md) 作为本地 AI 方案。
...
5. ✅ **AI 集成**：支持 OpenAI, Anthropic, DeepSeek 三大提供商
