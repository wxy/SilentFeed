# 页面重复学习问题分析

## 问题描述

用户发现系统会重复学习同一个页面，导致数据库中出现同一 URL 的多条访问记录。

## 当前机制分析

### 1. Content Script 层面（page-tracker.ts）

**去重机制**：
- 使用 `isRecorded` 标志防止单次页面会话中重复记录
- 问题：每次页面刷新或重新访问时，`isRecorded` 会被重置为 `false`

```typescript
// src/contents/page-tracker.ts:467
if (isRecorded) {
  logger.debug('🚫 [PageTracker] 已记录过，跳过')
  return
}
```

**SPA 处理**：
- 监听 URL 变化（`popstate`、`pushstate`、`replacestate`）
- URL 变化时会重置追踪状态
- 这意味着在同一个网站内导航会被视为不同的访问

### 2. Background 层面（background.ts）

**保存逻辑**：
```typescript
// src/background.ts:377
await db.confirmedVisits.add(visitData)
```

- 直接使用 `add()` 方法添加记录
- **没有任何 URL 去重检查**
- 每次接收到 `SAVE_PAGE_VISIT` 消息都会新增一条记录

### 3. SemanticProfileBuilder 层面

**去重机制**：
```typescript
// src/core/profile/SemanticProfileBuilder.ts:174-179
// 去重检查：5分钟内相同 URL 只处理一次
const lastBrowseTime = this.recentBrowses.get(page.url)
if (lastBrowseTime && (now - lastBrowseTime) < this.BROWSE_DEDUP_MS) {
  return
}
```

- 仅在画像学习层面去重（5分钟窗口）
- **不影响数据库记录**：页面仍然被保存到 `confirmedVisits`

### 4. 数据库设计（db/index.ts）

**索引设计**：
```typescript
confirmedVisits: 'id, domain, visitTime, *analysis.keywords, [visitTime+domain]'
```

- 主键是 `id`（UUID），每次都是唯一的
- **没有 URL 唯一性约束**
- 复合索引 `[visitTime+domain]` 允许同一域名多次访问

## 问题根源

1. **数据库层面**：`confirmedVisits` 表没有 URL 唯一性约束
2. **保存逻辑**：Background 直接 `add()` 而不检查是否已存在
3. **设计意图**：系统设计上**允许**重复访问同一个 URL

## 业务场景分析

### 场景 1：用户多次访问同一文章（内容未变化）

**现状**：
- 每次访问都会创建新的 `ConfirmedVisit` 记录
- 画像学习会被 5 分钟去重窗口限制

**问题**：
- 数据库冗余（同一内容被多次记录）
- 统计不准确（页面访问数被夸大）

**合理性**：
- ❌ 不合理：浪费存储空间
- ❌ 可能干扰推荐算法（重复的兴趣信号）

### 场景 2：用户多次访问同一 URL（内容已更新）

**现状**：
- 同样会创建多条记录
- 每条记录的 `analysis` 可能不同（如果内容变化了）

**合理性**：
- ✅ 合理：记录用户兴趣变化
- ✅ 合理：捕捉内容更新后的再次关注

**例子**：
- 新闻网站：同一 URL 但标题/内容更新
- GitHub Issue：持续关注某个 Issue 的进展
- 博客文章：作者更新了内容

### 场景 3：短时间内重复访问（例如刷新页面）

**现状**：
- 如果超过 30 秒阈值，会被记录两次
- 画像学习有 5 分钟去重保护

**问题**：
- ❌ 不合理：明显的误记录

## 解决方案

### 方案 A：严格去重（不推荐）

**实现**：
- 在 Background 保存前检查 URL 是否已存在
- 如果存在，更新 `visitTime` 和 `duration`

**优点**：
- 数据库干净，无冗余

**缺点**：
- ❌ 丢失访问历史（无法知道用户访问了几次）
- ❌ 无法追踪内容更新后的再次关注
- ❌ 统计信息不准确（最后一次访问时间，而非所有访问）

### 方案 B：时间窗口去重（推荐）⭐

**实现**：
- 在数据库保存前检查：
  - 同一 URL
  - 最近 N 分钟内（建议 30 分钟）
- 如果找到，更新现有记录的 `visitTime` 和 `duration`
- 如果没有或超过时间窗口，创建新记录

**优点**：
- ✅ 平衡了去重和历史追踪
- ✅ 保留长期访问历史
- ✅ 减少短期重复记录
- ✅ 符合用户预期（30 分钟内的多次访问算一次）

**实现示例**：
```typescript
// src/background.ts
case 'SAVE_PAGE_VISIT':
  const visitData = message.data
  
  // 检查最近 30 分钟内是否有相同 URL 的访问
  const thirtyMinutesAgo = Date.now() - 30 * 60 * 1000
  const recentVisit = await db.confirmedVisits
    .where('url').equals(visitData.url)
    .and(visit => visit.visitTime > thirtyMinutesAgo)
    .first()
  
  if (recentVisit) {
    // 更新现有记录
    await db.confirmedVisits.update(recentVisit.id, {
      visitTime: visitData.visitTime,
      duration: recentVisit.duration + visitData.duration,
      interactionCount: recentVisit.interactionCount + visitData.interactionCount
    })
  } else {
    // 创建新记录
    await db.confirmedVisits.add(visitData)
  }
```

### 方案 C：软去重 + 关联记录（复杂但最完整）

**实现**：
- 添加 `originalVisitId` 字段
- 首次访问创建主记录
- 后续访问创建子记录并关联到主记录
- 统计时可以选择是否包含子记录

**优点**：
- ✅ 保留完整访问历史
- ✅ 支持灵活的统计策略

**缺点**：
- ❌ 实现复杂
- ❌ 查询性能影响

### 方案 D：配置化（混合方案）

**实现**：
- 添加用户设置：
  - `deduplication: 'none' | 'strict' | 'window'`
  - `deduplicationWindow: number`（分钟）
- 根据用户选择应用不同策略

**优点**：
- ✅ 灵活性高
- ✅ 满足不同用户需求

**缺点**：
- ❌ 增加复杂度
- ❌ 用户可能不理解这些选项

## 推荐方案

**采用方案 B（时间窗口去重）**，配置如下：

1. **去重窗口**：30 分钟
   - 理由：与 Content Script 的 30 秒阈值对应，合理的短期去重期
   - 用户在 30 分钟内反复访问同一页面，视为同一次学习会话

2. **更新策略**：
   - 更新 `visitTime` 为最新时间
   - 累加 `duration`（总停留时间）
   - 累加 `interactionCount`
   - 保留原有的 `analysis`（假设内容未变化）

3. **特殊情况**：
   - 如果 `analysis` 内容发生重大变化（例如关键词完全不同），视为新内容，创建新记录
   - 这需要内容哈希或关键词相似度检测

## 实现步骤

1. ✅ 创建分支 `feature/page-revisit-handling`
2. ✅ 添加 URL 索引到 `confirmedVisits` 表（版本 17）
3. ✅ 实现时间窗口去重逻辑
4. ✅ 添加日志记录去重行为
5. ✅ 编写测试用例（6个测试，全部通过）
6. ⬜ 更新文档

## 实现详情

### 数据库变更

**版本 17** - 添加 URL 复合索引：
```typescript
confirmedVisits: 'id, url, visitTime, domain, *analysis.keywords, [visitTime+domain], [url+visitTime]'
```

新增复合索引 `[url+visitTime]` 支持高效的时间窗口查询。

### 去重逻辑

位置：`src/background.ts:374-417`

```typescript
// 30分钟窗口
const DEDUP_WINDOW_MS = 30 * 60 * 1000
const windowStart = visitData.visitTime - DEDUP_WINDOW_MS

// 查找最近访问
const recentVisit = await db.confirmedVisits
  .where('[url+visitTime]')
  .between([visitData.url, windowStart], [visitData.url, visitData.visitTime])
  .reverse()
  .first()

if (recentVisit) {
  // 更新现有记录
  await db.confirmedVisits.update(recentVisit.id, {
    visitTime: visitData.visitTime,
    duration: recentVisit.duration + visitData.duration,
    interactionCount: recentVisit.interactionCount + visitData.interactionCount
  })
} else {
  // 创建新记录
  await db.confirmedVisits.add(visitData)
}
```

### 测试覆盖

文件：`src/background/page-visit-dedup.test.ts`

**测试场景**：
1. ✅ 30分钟内合并相同 URL 的访问
2. ✅ 30分钟后创建新记录
3. ✅ 正确处理不同 URL 的访问
4. ✅ 窗口边界测试（29分59秒 vs 30分01秒）
5. ✅ 索引性能测试（100条记录，查询 <10ms）

所有测试通过，验证了去重逻辑的正确性和性能。

## 测试场景

1. **同一页面多次访问**（30分钟内）
   - 预期：只有一条记录，duration 累加

2. **同一页面多次访问**（30分钟后）
   - 预期：创建新记录

3. **不同页面访问**
   - 预期：正常创建多条记录

4. **页面刷新**（快速刷新）
   - 预期：如果超过30秒阈值，应该被去重

5. **SPA 导航**（站内跳转）
   - 预期：不同 URL，应该创建不同记录

## 配置选项（未来扩展）

可以考虑添加到 Settings：

```typescript
interface TrackingSettings {
  // 页面访问去重策略
  visitDeduplication: {
    enabled: boolean           // 是否启用去重
    windowMinutes: number      // 去重时间窗口（分钟）
    mergeContent: boolean      // 是否合并内容分析结果
  }
}
```

## 相关代码

- `src/contents/page-tracker.ts:467-500` - 记录页面访问
- `src/background.ts:300-420` - 保存访问记录
- `src/core/profile/SemanticProfileBuilder.ts:167-215` - 画像学习去重
- `src/storage/db/index.ts:80-200` - 数据库索引定义
